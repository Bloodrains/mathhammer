...
function Matrix( rowsOrMatrix, cols, defaultVal){
	...
	for(var x=0; x < Matrix.MAX_DIM ; x++)
		{
		this[x] = new Array();
		}
	...
	switch(checkArgs(arguments))
		...
		case -2 : 
				...
				if(rowsOrMatrix.constructor == Matrix)
					{
					for(var a=0; a < Math.min(rowsOrMatrix.getRowSize(),Matrix.MAX_DIM); a++)
						{
						for(var b=0;b< Math.min(rowsOrMatrix.getColSize(),Matrix.MAX_DIM);b++)
							{
				...
				// else if it is a String object...	
					...
					if(rowsOrMatrix.constructor == String)
						...
						for(var o=0; o < rowsOrMatrix.length; o++)
							{	
					...
					for(var c=0; c  < Math.min(rowsOrMatrix.length,Matrix.MAX_DIM); c++)
						{
						...
						for(var d=0; d < Math.min(rowsOrMatrix[c].length,Matrix.MAX_DIM); d++)
							{
...
Matrix.prototype.toString = function (){
	var matStr = "";
	for(var a=0; a < this.getRowSize(); a++)
		{
		matStr += this[a].join(", ") + "\n";
		}
...
Matrix.prototype.setRow = function ( rowIndex, newVals){
	...
	for(var a=0; a < newVals.length ; a++)
		{
...
Matrix.prototype.scale = function ( scaleBy){
		...
		for(var a=0; a < this.getRowSize() ; a++)
			{
			for(var b=0; b < this.getColSize() ; b++)
				{
...
Matrix.prototype.transpose = function (){
	...
	for(var a=0; a < tmpMat.getColSize(); a++)
		{
		for(var b=0; b < tmpMat.getRowSize(); b++)
			{
			this[a][b] = tmpMat[b][a];
			}
		}
...
Matrix.prototype.addOrSubtract = function ( toMatrix, sign){
	...
	for(var a=0; a < tmpMat.getRowSize(); a++)
		{
		for(var b=0; b < tmpMat.getColSize(); b++)
			{
...
Matrix.prototype.multiply = function ( byMatrix){
		if(byMatrix.constructor != Matrix || byMatrix.getRowSize() != this.getColSize())
			{
			return null;
			}
		...
		for(var i=0 ; i < tmpMat.getRowSize() ; i++)
			{
			for(var j=0 ; j < tmpMat.getColSize(); j++)
				{
